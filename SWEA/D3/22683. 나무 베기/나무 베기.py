...
for testcase in range(1, T + 1):
    ...
    def dijkstra(start_col, start_row, cut_cnt):
        """
        다익스트라 탐색
        ...
        :return:
        """
        # 우선순위 큐를 사용하여 최소 비용 경로 탐색
        # 열, 행, 방향, 남은 cut_cnt, 리모컨 조작 횟수
        heap = [(0, start_col, start_row, 0, cut_cnt)]  # 리모컨 조작 횟수, 현재 열, 행, 방향, 남은 cut_cnt

        # 상, 우, 하, 좌 방향
        DIRECTIONS = [(-1, 0), (0, 1), (1, 0), (0, -1)]

        while heap:
            remote_cnt, current_col, current_row, current_view, remaining_cut_cnt = heapq.heappop(heap)
            ...
            if 0 <= next_col < N and 0 <= next_row < N:
                ...
                if matrix[next_col][next_row] == GROUND_MOVE or matrix[next_col][next_row] == ARRIVE_LOCATION:
                    if distances[next_col][next_row][current_view][remaining_cut_cnt] > remote_cnt + 1:
                        ...
                        heapq.heappush(heap, (remote_cnt + 1, next_col, next_row, current_view, remaining_cut_cnt))
                ...
                # TREE_CANT_MOVE로 이동하려면 나무를 베어야 함
                    ...
                    if distances[next_col][next_row][current_view][remaining_cut_cnt - 1] > remote_cnt + 1:
                        ...
                        heapq.heappush(heap, (remote_cnt + 1, next_col, next_row, current_view, remaining_cut_cnt - 1))
            ...
            for new_view in new_views:
                if distances[current_col][current_row][new_view][remaining_cut_cnt] > remote_cnt + 1:
                    ...
                    heapq.heappush(heap, (remote_cnt + 1, current_col, current_row, new_view, remaining_cut_cnt))

    dijkstra(start_col, start_row, cut_cnt)
